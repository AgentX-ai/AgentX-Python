...
def build_single_agent():
    ...
    def run(query: str):
        ...
        output = next((m.get("content", "") for m in reversed(messages) if m.get("role") == "assistant"), "")
...
def build_group_chat():
    ...
    def run(query: str):
        group_chat = GroupChat(agents=[user_proxy, researcher, writer], messages=[], max_round=4)
        ...
        for msg in group_chat.messages:
            if msg.get("role") == "assistant" and msg.get("name") in ("Researcher", "Writer"):
                trace_events.append({
                    "type": "agent_step",
                    "name": msg["name"].lower(),
        ...
        output = next(
            (m.get("content", "") for m in reversed(group_chat.messages) if m.get("name") == "Writer"),
            "",
        )
        input_tokens, output_tokens = _extract_tokens(chat_result)
...
def main():
    ...
    display = "AutoGen GroupChat (researcher + writer)" if mode == "group" else "AutoGen Single Agent"
