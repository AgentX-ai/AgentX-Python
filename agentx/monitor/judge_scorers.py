...
class JudgeScorersClient:
    ...
    def __init__(
        ...
        # Same workspace pinning MonitorClient does - without it, scorer CRUD silently lands
    ...
    def _request(self, method: str, path: str, json: Any = None, timeout: int = 60) -> Any:
        ...
        if resp.status_code >= 400:
            ...
            raise AgentXJudgeScorersError(
                f"Judge scorer request failed ({resp.status_code}): {detail}",
    ...
    def builder(
        ...
        *,
        ...
        if idle_seconds is not None and not live:
            # The online profile is only built when live=True - an explicit idle_seconds
            # would otherwise be silently dropped.
            raise ValueError("idle_seconds requires live=True")
        if not live:
            # Same guard for the rest of the online profile: agent_ids and a non-default
            # scope only reach the wire when live=True - explicit values would otherwise
            # be silently discarded.
            offending = [
                kwarg
                for kwarg, given in (("agent_ids", bool(agent_ids)), ("scope", scope != "trace"))
                if given
            ]
            if offending:
                verb = "require" if len(offending) > 1 else "requires"
                raise ValueError(f"{' and '.join(offending)} {verb} live=True")
        judge: Dict[str, Any] = {}
        for key, value in (
